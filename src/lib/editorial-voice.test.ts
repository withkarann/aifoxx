...
// The guard lives with the build tooling; the test rides along in src/ so vitest
// picks it up. It protects the promise that every trust report is buyer-facing.
// @ts-expect-error -- plain .mjs module, no types
import { GUARD_RE, AUDIT_RE, findEditorialVoice } from "../../scripts/editorial-voice.mjs";

describe("editorial-voice GUARD_RE", () => {
  // Phrases that leaked onto live trust pages and must always be caught.
  const internal = [
    "QA CORRECTION (2026-06-27): the prior ISO entry used a paraphrased quote.",
    "No public evidence; graded held=false with no vendor-domain source.",
  ...
  // Genuine buyer-facing copy that must never trip the guard.
  const clean = [
...
describe("findEditorialVoice", () => {
  it("returns a hit for a report with an internal-voiced flag", () => {
    const report = {
      slug: "acme",
      flags: [
        "Trains on customer data by default; opt-out available.",
        "QA CORRECTION (2026-06-27): certification list was re-scoped.",
      ],
